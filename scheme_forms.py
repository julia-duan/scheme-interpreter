...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        env.define(expressions.first, scheme_eval(expressions.rest.first, env)) #env = not self, but bc define comes from FRame
        return expressions.first #have to return this
        # END PROBLEM 4
    elif isinstance(signature, Pair) and scheme_symbolp(signature.first):
        # defining a named procedure e.g. (define (f x y) (+ x y))
        # BEGIN PROBLEM 10
        # print("DEBUG: ", signature.first) prints f??????
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    return LambdaProcedure(formals, expressions.rest, env) # creating an object # do not need self bc not object oriented
    # END PROBLEM 7
...
def do_and_form(expressions, env):
    ...
    #expressions = operands... do NOT need to calc_eval ALL of them bc short-circuiting
    if expressions is nil:
        return True #if nothing in the (and ... ) then ofc it will be true
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False #if nothing in the (and ... ) then ofc it will be true
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            # BEGIN PROBLEM 13
            # pay attention to what they give you
            if clause.rest is nil:
                return test
            else:
                return eval_all(clause.rest, env) #returns NOne if... 

            # while expressions:
            #     if expressions.first.first == 'else':
            #         if expressions.first.rest is nil:
            #             return True
            #         else:
            #             return eval_all(expressions.first.rest, env)
            #         eval_all(expressions.rest, env)
            #     elif is_scheme_true(expressions.first.first): #error: repetition in evaluating this line... smth to do with while
            #         if expressions.first.rest is nil:
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        names = Pair(bindings.first.first, names) #right to left... technically
